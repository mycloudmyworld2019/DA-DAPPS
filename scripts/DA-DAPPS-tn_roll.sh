#!/bin/bash
#
# DA-DAPPSio-tn_roll is used to have all of the instances of the EOS daemon on a host brought down
# so that the underlying executable image file (the "text file") can be replaced. Then
# all instances are restarted.
# usage: DA-DAPPSio-tn_roll.sh [arglist]
...
# the command line.
#
# In most cases, simply running ./DA-DAPPSio-tn_roll.sh is sufficient.
#

if [ -z "$DA-DAPPSio_HOME" ]; then
    echo DA-DAPPSio_HOME not set - $0 unable to proceed.
    exit -1
fi
...
prog=""
RD=""
for p in eosd DA-DAPPSiod nodeos; do
    prog=$p
    RD=bin
    if [ -f $RD/$prog ]; then
        break;
...
if [ \( -z "$prog" \) -o \( -z "$RD" \) ]; then
    echo unable to locate binary for eosd or DA-DAPPSiod or nodeos
    exit 1
fi

SDIR=staging/DA-DAPPS
...
if [ $DD = "all" ]; then
    for DA-DAPPSio_RESTART_DATA_DIR in `ls -d var/lib/node_??`; do
        bash $DA-DAPPSio_HOME/scripts/DA-DAPPSio-tn_up.sh "$*"
    done
else
    bash $DA-DAPPSio_HOME/scripts/DA-DAPPSio-tn_up.sh "$*"
fi
unset DA-DAPPSio_RESTART_DATA_DIR
