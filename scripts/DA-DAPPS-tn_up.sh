#!/bin/bash
#
# DA-DAPPSio-tn_up is a helper script used to start a node that was previously stopped.
# It is not intended to be run stand-alone; it is a companion to the
# DA-DAPPSio-tn_bounce.sh and DA-DAPPSio-tn_roll.sh scripts.
...
datadir=var/lib/node_$DA-DAPPSio_NODE
...
relaunch() {
    echo "$rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/DA-DAPPSio/node_$DA-DAPPSio_NODE > $datadir/stdout.txt  2>> $datadir/$log "
    nohup $rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/DA-DAPPSio/node_$DA-DAPPSio_NODE > $datadir/stdout.txt  2>> $datadir/$log &
    pid=$!
    echo pid = $pid
...
if [ -z "$DA-DAPPSio_LEVEL" ]; then
    echo starting with no modifiers
    relaunch
    if [ "$connected" -eq 0 ]; then
        DA-DAPPSio_LEVEL=replay
    else
        exit 0
    fi
fi

if [ "$DA-DAPPSio_LEVEL" == replay ]; then
    echo starting with replay
    relaunch --hard-replay-blockchain
    if [  "$connected" -eq 0 ]; then
        DA-DAPPSio_LEVEL=resync
    else
        exit 0
    fi
fi
if [ "$DA-DAPPSio_LEVEL" == resync ]; then
