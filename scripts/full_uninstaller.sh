...
if [ -d "/usr/local/include/DA-DAPPSio" ] || [ -d "$HOME/opt/DA-DAPPSio" ] || [ $FORCED == 1 ]; then # use force for running the script directly
   printf "\nDA-DAPPSio installation (AND DEPENDENCIES) already found...\n"
   if [ $1 == 0 ]; then
   ...
   case $ANSWER in
      1 | [Yy]* )
         if [ -d "$HOME/opt/DA-DAPPSio" ] || [[ $1 == "force-new" ]]; then
            if [ $( uname ) == "Darwin" ]; then
               # gettext and other brew packages are not modified as they can be dependencies for things other than DA-DAPPSio
            ...
            rm -rf $HOME/opt/DA-DAPPSio
            rm -f $HOME/bin/DA-DAPPSio-launcher
            rm -rf $HOME/lib/cmake/DA-DAPPSios
            rm -rf $HOME/opt/llvm
            rm -f $HOME/opt/boost
         ...
         if [ -d "/usr/local/include/DA-DAPPSio" ] || [[ $1 == "force-old" ]]; then
            if [ "$(id -u)" -ne 0 ]; then
            ...
            rm -rf libbson-1.0 libmongoc-1.0 mongocxx bsoncxx appbase chainbase DA-DAPPSio.system DA-DAPPSiolib fc libc++ musl secp256k* 2>/dev/null
            rm -rf DA-DAPPSio 2>/dev/null
            popd &> /dev/null
            pushd bin &> /dev/null
            rm cleos DA-DAPPSio-abigen DA-DAPPSio-applesedemo DA-DAPPSio-launcher DA-DAPPSio-s2wasm DA-DAPPSio-wast2wasm DA-DAPPSiocpp keosd nodeos 2>/dev/null
            popd &> /dev/null
            libraries=(
               libDA-DAPPSio_testing
               libDA-DAPPSio_chain
               libfc
               libbinaryen
               libWAST
