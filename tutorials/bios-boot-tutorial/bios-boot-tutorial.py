...
fastUnstakeSystem = './fast.refund/DA-DAPPSio.system/DA-DAPPSio.system.wasm'
...
def startNode(nodeIndex, account):
    ...
    if not nodeIndex: otherOpts += (
        '    --plugin DA-DAPPSio::history_plugin'
    ...
    cmd = (
        ...
        '    --plugin DA-DAPPSio::http_plugin'
        '    --plugin DA-DAPPSio::chain_api_plugin'
        '    --plugin DA-DAPPSio::producer_plugin' +
        otherOpts)
...
def createSystemAccounts():
    for a in systemAccounts:
        run(args.cleos + 'create account DA-DAPPSio ' + a + ' ' + args.public_key)
...
def createStakedAccounts(b, e):
    ...
    for i in range(b, e):
        ...
        retry(args.cleos + 'system newaccount --transfer DA-DAPPSio %s %s --stake-net "%s" --stake-cpu "%s" --buy-ram "%s"   ' % 
            (a['name'], a['pub'], intToCurrency(stakeNet), intToCurrency(stakeCpu), intToCurrency(ramFunds)))
        if unstaked:
            retry(args.cleos + 'transfer DA-DAPPSio %s "%s"' % (a['name'], intToCurrency(unstaked)))
...
def claimRewards():
    table = getJsonOutput(args.cleos + 'get table DA-DAPPSio DA-DAPPSio producers -l 100')
    times = []
...
def updateAuth(account, permission, parent, controller):
    run(args.cleos + 'push action DA-DAPPSio updateauth' + jsonArg({
        'account': account,
...
def msigProposeReplaceSystem(proposer, proposalName):
    ...
    trxPermissions = [{'actor': 'DA-DAPPSio', 'permission': 'active'}]
    with open(fastUnstakeSystem, mode='rb') as f:
        setcode = {'account': 'DA-DAPPSio', 'vmtype': 0, 'vmversion': 0, 'code': f.read().hex()}
    run(args.cleos + 'multisig propose ' + proposalName + jsonArg(requestedPermissions) + 
        jsonArg(trxPermissions) + 'DA-DAPPSio setcode' + jsonArg(setcode) + ' -p ' + proposer)
...
def msigReplaceSystem():
    run(args.cleos + 'push action DA-DAPPSio buyrambytes' + jsonArg(['DA-DAPPSio', accounts[0]['name'], 200000]) + '-p DA-DAPPSio')
    sleep(1)
...
def stepStartWallet():
    ...
    importKeys()
def stepStartBoot():
    startNode(0, {'name': 'DA-DAPPSio', 'pvt': args.private_key, 'pub': args.public_key})
    sleep(1.5)
def stepInstallSystemContracts():
    run(args.cleos + 'set contract DA-DAPPSio.token ' + args.contracts_dir + '/DA-DAPPSio.token/')
    run(args.cleos + 'set contract DA-DAPPSio.msig ' + args.contracts_dir + '/DA-DAPPSio.msig/')
def stepCreateTokens():
    run(args.cleos + 'push action DA-DAPPSio.token create \'["DA-DAPPSio", "10000000000.0000 %s"]\' -p DA-DAPPSio.token' % (args.symbol))
    totalAllocation = allocateFunds(0, len(accounts))
    run(args.cleos + 'push action DA-DAPPSio.token issue \'["DA-DAPPSio", "%s", "memo"]\' -p DA-DAPPSio' % intToCurrency(totalAllocation))
    sleep(1)
def stepSetSystemContract():
    retry(args.cleos + 'set contract DA-DAPPSio ' + args.contracts_dir + '/DA-DAPPSio.system/')
    sleep(1)
    run(args.cleos + 'push action DA-DAPPSio setpriv' + jsonArg(['DA-DAPPSio.msig', 1]) + '-p DA-DAPPSio@active')
def stepInitSystemContract():
    run(args.cleos + 'push action DA-DAPPSio init' + jsonArg(['0', '4,SYS']) + '-p DA-DAPPSio@active')
    sleep(1)
...
def stepResign():
    resign('DA-DAPPSio', 'DA-DAPPSio.prods')
    for a in systemAccounts:
        resign(a, 'DA-DAPPSio')
def stepTransfer():
    while True:
...
def stepLog():
    run('tail -n 60 ' + args.nodes_dir + '00-DA-DAPPSio/stderr')
...
commands = [
    ...
    ('s', 'sys',                createSystemAccounts,       True,    "Create system accounts (DA-DAPPSio.*)"),
    ...
    ('q', 'resign',             stepResign,                 True,    "Resign DA-DAPPSio"),
    ('m', 'msg-replace',        msigReplaceSystem,          False,   "Replace system contract using msig"),
    ('X', 'xfer',               stepTransfer,               False,   "Random transfer tokens (infinite loop)"),
    ('l', 'log',                stepLog,                    True,    "Show tail of node's log"),
]

parser.add_argument('--public-key', metavar='', help="DA-DAPPSio Public Key", default='DA-DAPPS8Znrtgwt8TfpmbVpTKvA2oB8Nqey625CLN8bCN3TEbgx86Dsvr', dest="public_key")
parser.add_argument('--private-Key', metavar='', help="DA-DAPPSio Private Key", default='5K463ynhZoCDDa4RDcr63cUwWLTnKqmdcoTKTHBjqoKfv4u5V7p', dest="private_key")
...
parser.add_argument('--symbol', metavar='', help="The DA-DAPPSio.system symbol", default='SYS')
