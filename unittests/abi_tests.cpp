...
#include <DA-DAPPSio/chain/eosio_contract.hpp>
...
BOOST_AUTO_TEST_CASE(abi_large_array)
{
   try {
      ...
      // curl http://127.0.0.1:8888/v1/chain/abi_bin_to_json -X POST -d '{"code":"DA-DAPPSio", "action":"hi", "binargs":"ffffffff08"}'
...
BOOST_AUTO_TEST_CASE(abi_recursive_structs)
{
   try {
      ...
      abi_serializer abis(fc::json::from_string(abi_str).as<abi_def>(), max_serialization_time);
      string hi_data = "{\"user\":\"DA-DAPPSio\"}";
...
{
   using DA-DAPPSio::testing::fc_exception_message_starts_with;

   auto duplicate_variant_abi = R"({
      "version": "DA-DAPPSio::abi/1.1",
      "variants": [
...
BOOST_AUTO_TEST_CASE(version)
{
   try {
      ...
      BOOST_CHECK_THROW( abi_serializer(fc::json::from_string(R"({"version": "DA-DAPPSio::abi/9.0"})").as<abi_def>(), max_serialization_time), unsupported_abi_version_exception );
      abi_serializer(fc::json::from_string(R"({"version": "DA-DAPPSio::abi/1.0"})").as<abi_def>(), max_serialization_time);
      abi_serializer(fc::json::from_string(R"({"version": "DA-DAPPSio::abi/1.1"})").as<abi_def>(), max_serialization_time);
   } FC_LOG_AND_RETHROW()
}
...
{
   using DA-DAPPSio::testing::fc_exception_message_is;
