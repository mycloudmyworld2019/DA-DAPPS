...
public:
   ...
   eosio_token_tester() {
      ...
      create_accounts( { N(alice), N(bob), N(carol), N(DA-DAPPSio.token) } );
      produce_blocks( 2 );
   ...
   fc::variant get_account( account_name acc, const string& symbolname)
   {
      auto symb = DA-DAPPSio::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( N(DA-DAPPSio.token), acc, N(accounts), symbol_code );
   ...
   action_result create( account_name issuer,
      ...
      return push_action( N(DA-DAPPSio.token), N(create), mvo()
           ( "issuer", issuer)
