...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   // Link authority for DA-DAPPSio reqauth action with alice's spending key
   chain.link_authority("alice", "DA-DAPPSio", "spending",  "reqauth");
   ...
   BOOST_CHECK_THROW( chain.link_authority("alice", "DA-DAPPSio", "spending",  "reqauth"), action_validate_exception);

   // Unlink alice with DA-DAPPSio reqauth
   chain.unlink_authority("alice", "DA-DAPPSio", "reqauth");
   ...
   // Link authority for any DA-DAPPSio action with alice's scud key
   chain.link_authority("alice", "DA-DAPPSio", "scud");
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority("alice", "DA-DAPPSio", "first",  "reqauth");
...
try {
   ...
   // Creating account with DA-DAPPSio. prefix with privileged account
   chain.create_account("DA-DAPPSio.test1");

   // Creating account with DA-DAPPSio. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account("DA-DAPPSio.test2", "joe"), action_validate_exception,
                         fc_exception_message_is("only privileged accounts can have names that start with 'DA-DAPPSio.'"));
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( "alice", "DA-DAPPSio", "DA-DAPPSio.any", "reqauth" );
   chain.link_authority( "bob", "DA-DAPPSio", "DA-DAPPSio.any", "reqauth" );

   /// this should succeed because DA-DAPPSio::reqauth is linked to any permission
   chain.push_reqauth("alice", { permission_level{N(alice), "spending"} }, { spending_priv_key });
...
try {
   ...
   using resource_usage_object = DA-DAPPSio::chain::resource_limits::resource_usage_object;
   using by_owner = DA-DAPPSio::chain::resource_limits::by_owner;
...
BOOST_AUTO_TEST_CASE( linkauth_special ) { try {
   ...
   auto validate_disallow = [&] (const char *type) {
      BOOST_REQUIRE_EXCEPTION(
         ...
         fc_exception_message_is(std::string("Cannot link DA-DAPPSio::") + std::string(type) + std::string(" to a minimum permission"))
      );
   };
