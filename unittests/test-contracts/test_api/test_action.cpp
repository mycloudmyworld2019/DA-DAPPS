...
void test_action::test_dummy_action() {
   ...
   DA-DAPPSio::action act = DA-DAPPSio::get_action( 1, 0 );
   ...
   eosio_assert( DA-DAPPSio::pack_size(act) == static_cast<size_t>(total), "pack_size does not match get_action size" );
...
void test_action::test_cf_action() {
   ...
   DA-DAPPSio::action act = DA-DAPPSio::get_action( 0, 0 );
   ...
   if ( cfa.payload == 100 ) {
      ...
      uint32_t v = DA-DAPPSio::unpack<uint32_t>( &cfd[0], cfd.size() );
...
void test_action::require_notice( uint64_t receiver, uint64_t code, uint64_t action ) {
   ...
   if( receiver == "testapi"_n.value ) {
      DA-DAPPSio::require_recipient( "acc1"_n );
      DA-DAPPSio::require_recipient( "acc2"_n );
      DA-DAPPSio::require_recipient( "acc1"_n, "acc2"_n );
...
void test_action::require_notice_tests( uint64_t receiver, uint64_t code, uint64_t action ) {
   DA-DAPPSio::print( "require_notice_tests" );
   if( receiver == "testapi"_n.value ) {
      DA-DAPPSio::print("require_recipient( \"acc5\"_n )");
      DA-DAPPSio::require_recipient("acc5"_n);
   } else if( receiver == "acc5"_n.value ) {
      DA-DAPPSio::print("require_recipient( \"testapi\"_n )");
      DA-DAPPSio::require_recipient("testapi"_n);
   }
}

void test_action::require_auth() {
   prints("require_auth");
   DA-DAPPSio::require_auth("acc3"_n);
   DA-DAPPSio::require_auth("acc4"_n);
}
...
void test_action::test_ram_billing_in_notify( uint64_t receiver, uint64_t code, uint64_t action ) {
   ...
   if( code == receiver ) {
      DA-DAPPSio::require_recipient( name{to_notify} );
   } else {
