...
struct test_action_action {
   static DA-DAPPSio::name get_account() {
      return DA-DAPPSio::name{ACCOUNT};
   }

   static DA-DAPPSio::name get_name() {
      return DA-DAPPSio::name{NAME};
   }
...
void test_transaction::assert_false_error_handler( const DA-DAPPSio::transaction& dtrx ) {
...
void test_transaction::deferred_print() {
   DA-DAPPSio::print("deferred executed\n");
}
...
void test_transaction::stateful_api() {
   char buf[4] = {1};
   db_store_i64( DA-DAPPSio::name{"testtrans"}.value, DA-DAPPSio::name{"table"}.value, DA-DAPPSio::name{"testtrans"}.value, 0, buf, 4 );
}
...
void test_transaction::repeat_deferred_transaction( uint64_t receiver, uint64_t code, uint64_t action ) {
   ...
   trx.send( sender_id, DA-DAPPSio::name{receiver} );
}
