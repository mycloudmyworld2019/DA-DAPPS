...
using namespace DA-DAPPSio::internal_use_do_not_use;
...
void test_api_db::test_invalid_access( name _code, uint64_t val, uint32_t index, bool store )
{
   ...
   if(store) {
      ...
      //DA-DAPPSio::print("test_invalid_access: stored ", value_to_store, "\n");
   } else {
      ...
      //DA-DAPPSio::print("test_invalid_access: expected ", val, " and retrieved ", value, "\n");
...
void test_api_db::misaligned_secondary_key256_tests() {
   uint64_t receiver = get_self().value;
   auto key = DA-DAPPSio::checksum256::make_from_word_sequence<uint64_t>( 0ULL, 0ULL, 0ULL, 42ULL );
   char* ptr = (char*)(&key);
   ptr += 1;
