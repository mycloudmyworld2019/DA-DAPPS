 ...
 */
#include "proxy.hpp"
#include <DA-DAPPSio/transaction.hpp>

using namespace DA-DAPPSio;

proxy::proxy( DA-DAPPSio::name self, DA-DAPPSio::name first_receiver, DA-DAPPSio::datastream<const char*> ds )
...
void proxy::on_transfer( name from, name to, asset quantity, const std::string& memo ) {
   ...
   if( from == self ) {
      ...
      transaction out;
      DA-DAPPSio::token::transfer_action a( "DA-DAPPSio.token"_n, {self, "active"_n} );
...
void proxy::on_error( uint128_t sender_id, DA-DAPPSio::ignore<std::vector<char>> ) {
