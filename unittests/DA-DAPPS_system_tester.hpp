...
public:
   ...
   eosio_system_tester(Lambda setup) {
      ...
      set_code( N(DA-DAPPSio.token), contracts::eosio_token_wasm() );
      set_abi( N(DA-DAPPSio.token), contracts::eosio_token_abi().data() );

      {
         const auto& accnt = control->db().get<account_object,by_name>( N(DA-DAPPSio.token) );
         abi_def abi;
         BOOST_REQUIRE_EQUAL(abi_serializer::to_abi(accnt.abi, abi), true);
         token_abi_ser.set_abi(abi, abi_serializer_max_time);
      }

      create_currency( N(DA-DAPPSio.token), config::system_account_name, core_from_string("10000000000.0000") );
      issue(config::system_account_name,      core_from_string("1000000000.0000"));
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance( "DA-DAPPSio" ) );
      ...
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance("DA-DAPPSio")  + get_balance("DA-DAPPSio.ramfee") + get_balance("DA-DAPPSio.stake") + get_balance("DA-DAPPSio.ram") );
   }
   ...
   asset get_balance( const account_name& act ) {
      vector<char> data = get_row_by_account( N(DA-DAPPSio.token), act, N(accounts), symbol(CORE_SYMBOL).to_symbol_code().value );
   ...
   void issue( name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(DA-DAPPSio.token), N(issue), manager, mutable_variant_object()
                                ("to",      to )
                                ("quantity", amount )
                                ("memo", "")
                                );
   ...
   void transfer( name from, name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(DA-DAPPSio.token), N(transfer), manager, mutable_variant_object()
                                ("from",    from)
                                ("to",      to )
                                ("quantity", amount)
                                ("memo", "")
   ...
   fc::variant get_stats( const string& symbolname ) {
      auto symb = DA-DAPPSio::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( N(DA-DAPPSio.token), symbol_code, N(stat), symbol_code );
   ...
   abi_serializer initialize_multisig() {
      ...
      {
         create_account_with_resources( N(DA-DAPPSio.msig), config::system_account_name );
         BOOST_REQUIRE_EQUAL( success(), buyram( "DA-DAPPSio", "DA-DAPPSio.msig", core_from_string("5000.0000") ) );
         produce_block();

         auto trace = base_tester::push_action(config::system_account_name, N(setpriv),
                                               config::system_account_name,  mutable_variant_object()
                                               ("account", "DA-DAPPSio.msig")
                                               ("is_priv", 1)
         );

         set_code( N(DA-DAPPSio.msig), contracts::eosio_msig_wasm() );
         set_abi( N(DA-DAPPSio.msig), contracts::eosio_msig_abi().data() );

         produce_blocks();
         const auto& accnt = control->db().get<account_object,by_name>( N(DA-DAPPSio.msig) );
         abi_def msig_abi;
   ...
   vector<name> active_and_vote_producers() {
      //stake more than 15% of total DA-DAPPS supply to activate chain
      transfer( "DA-DAPPSio", "alice1111111", core_from_string("650000000.0000"), "DA-DAPPSio" );
