...
public:
   ...
   fc::variant get_global_state() {
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance(N(DA-DAPPSio.token), symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // Create DA-DAPPSio.msig and DA-DAPPSio.token
        create_accounts({N(DA-DAPPSio.msig), N(DA-DAPPSio.token), N(DA-DAPPSio.ram), N(DA-DAPPSio.ramfee), N(DA-DAPPSio.stake), N(DA-DAPPSio.vpay), N(DA-DAPPSio.bpay), N(DA-DAPPSio.saving) });
        // Set code for the following accounts:
        //  - DA-DAPPSio (code: DA-DAPPSio.bios) (already set by tester constructor)
        //  - DA-DAPPSio.msig (code: DA-DAPPSio.msig)
        //  - DA-DAPPSio.token (code: DA-DAPPSio.token)
        // set_code_abi(N(DA-DAPPSio.msig), contracts::eosio_msig_wasm(), contracts::eosio_msig_abi().data());//, &eosio_active_pk);
        // set_code_abi(N(DA-DAPPSio.token), contracts::eosio_token_wasm(), contracts::eosio_token_abi().data()); //, &eosio_active_pk);

        set_code_abi(N(DA-DAPPSio.msig),
                     contracts::eosio_msig_wasm(),
                     contracts::eosio_msig_abi().data());//, &eosio_active_pk);
        set_code_abi(N(DA-DAPPSio.token),
                     contracts::eosio_token_wasm(),
                     contracts::eosio_token_abi().data()); //, &eosio_active_pk);

        // Set privileged for DA-DAPPSio.msig and DA-DAPPSio.token
        set_privileged(N(DA-DAPPSio.msig));
        set_privileged(N(DA-DAPPSio.token));

        // Verify DA-DAPPSio.msig and DA-DAPPSio.token is privileged
        const auto& eosio_msig_acc = get<account_object, by_name>(N(DA-DAPPSio.msig));
        BOOST_TEST(eosio_msig_acc.privileged == true);
        const auto& eosio_token_acc = get<account_object, by_name>(N(DA-DAPPSio.token));
        BOOST_TEST(eosio_token_acc.privileged == true);


        // Create SYS tokens in DA-DAPPSio.token, set its manager as DA-DAPPSio
        ...
        create_currency(N(DA-DAPPSio.token), config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion SYS tokens to DA-DAPPSio.system
        issue(N(DA-DAPPSio.token), config::system_account_name, config::system_account_name, initial_supply);
        ...
        for( const auto& a : test_genesis ) {
           ...
           r = delegate_bandwidth(N(DA-DAPPSio.stake), a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == "DA-DAPPSio");
