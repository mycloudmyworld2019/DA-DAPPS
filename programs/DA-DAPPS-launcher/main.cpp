...
#include <net/if.h>
#include <DA-DAPPSio/chain/genesis_state.hpp>
...
struct server_name_def {
  ...
  string DA-DAPPSio_home;
  uint16_t instances;
  server_name_def () : ipaddr(), name(), has_bios(false), DA-DAPPSio_home(), instances(1) {}
};
...
struct server_identities {
  ...
  vector<string> db;
  string default_DA-DAPPSio_home;
  remote_deploy ssh;
};
...
struct launcher_def {
   ...
   DA-DAPPSio::chain::genesis_state genesis_from_file;
...
launcher_def::initialize (const variables_map &vmap) {
  ...
  config_dir_base = "etc/DA-DAPPSio";
  ...
  char* erd_env_var = getenv ("DA-DAPPSio_HOME");
  ...
  stage = bfs::path(erd);
  if (!bfs::exists(stage)) {
    cerr << "\"" << erd << "\" is not a valid path. Please ensure environment variable DA-DAPPSio_HOME is set to the build path." << endl;
    exit (-1);
  }
...
launcher_def::define_network () {
  ...
  if (per_host == 0) {
    host_def local_host;
    local_host.DA-DAPPSio_home = erd;
    ...
    for (size_t i = total_nodes; i > 0; i--) {
      ...
      if (ph_count == 0) {
        ...
        lhost->DA-DAPPSio_home =
          (local_id.contains (lhost->host_name) || servers.default_DA-DAPPSio_home.empty()) ?
          erd : servers.default_DA-DAPPSio_home;
        host_ndx++;
      } // ph_count == 0
...
launcher_def::deploy_config_files (tn_node_def &node) {
  ...
  if (host->is_local()) {
    bfs::path cfgdir = bfs::path(host->DA-DAPPSio_home) / instance.config_dir_name;
    bfs::path dd = bfs::path(host->DA-DAPPSio_home) / instance.data_dir_name;
    ...
    bfs::path rfile = bfs::path (host->DA-DAPPSio_home) / instance.config_dir_name / "config.ini";
    ...
    rfile = bfs::path (host->DA-DAPPSio_home) / instance.config_dir_name / "logging.json";
    ...
    rfile = bfs::path (host->DA-DAPPSio_home) / instance.config_dir_name / "genesis.json";
...
launcher_def::write_config_file (tn_node_def &node) {
  ...
  if (instance.has_db || node.producers.size()) {
    ...
    cfg << "plugin = DA-DAPPSio::producer_plugin\n";
  }
  if( instance.has_db ) {
    cfg << "plugin = DA-DAPPSio::mongo_db_plugin\n";
  }
  if ( p2p == p2p_plugin::NET ) {
    cfg << "plugin = DA-DAPPSio::net_plugin\n";
  } else {
    cfg << "plugin = DA-DAPPSio::bnet_plugin\n";
  }
  cfg << "plugin = DA-DAPPSio::chain_api_plugin\n"
      << "plugin = DA-DAPPSio::history_api_plugin\n";
  cfg.close();
}
...
launcher_def::init_genesis () {
   ...
   if (!bfs::exists(genesis_path)) {
      ...
      DA-DAPPSio::chain::genesis_state default_genesis;
      fc::json::save_to_file( default_genesis, genesis_path, true );
   }
   string bioskey = string(network.nodes["bios"].keys[0].get_public_key());

   fc::json::from_file(genesis_path).as<DA-DAPPSio::chain::genesis_state>(genesis_from_file);
...
launcher_def::prep_remote_config_dir (eosd_def &node, host_def *host) {
  bfs::path abs_config_dir = bfs::path(host->DA-DAPPSio_home) / node.config_dir_name;
  bfs::path abs_data_dir = bfs::path(host->DA-DAPPSio_home) / node.data_dir_name;

  string acd = abs_config_dir.string();
  string add = abs_data_dir.string();
  string cmd = "cd " + host->DA-DAPPSio_home;

  cmd = "cd " + host->DA-DAPPSio_home;
  if (!do_ssh(cmd, host->host_name)) {
    cerr << "Unable to switch to path " << host->DA-DAPPSio_home
...
launcher_def::launch (eosd_def &instance, string &gts) {
  ...
  if (!eosd_extra_args.empty()) {
    if (instance.name == "bios") {
       ...
       regex r("--plugin +DA-DAPPSio::mongo_db_plugin");
  ...
  if (!host->is_local()) {
    string cmdl ("cd ");
    cmdl += host->DA-DAPPSio_home + "; nohup " + eosdcmd + " > "
    ...
    string cmd = "cd " + host->DA-DAPPSio_home + "; kill -15 $(cat " + pidf.string() + ")";
...
launcher_def::do_command(const host_def& host, const string& name,
   ...
   if (!host.is_local()) {
      string rcmd = "cd " + host.DA-DAPPSio_home + "; ";
...
launcher_def::bounce (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string cmd = "./scripts/DA-DAPPSio-tn_bounce.sh " + eosd_extra_args;
      ...
      do_command(host, node.name, { { "DA-DAPPSio_HOME", host.DA-DAPPSio_home }, { "DA-DAPPSio_NODE", node_num } }, cmd);
   }
}
...
launcher_def::down (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string cmd = "./scripts/DA-DAPPSio-tn_down.sh ";
      do_command(host, node.name,
                 { { "DA-DAPPSio_HOME", host.DA-DAPPSio_home }, { "DA-DAPPSio_NODE", node_num }, { "DA-DAPPSio_TN_RESTART_CONFIG_DIR", node.config_dir_name } },
                 cmd);
   }
}
...
launcher_def::roll (const string& host_names) {
   ...
   for (string host_name: hosts) {
      ...
      string cmd = "./scripts/DA-DAPPSio-tn_roll.sh ";
      do_command(*host, host_name, { { "DA-DAPPSio_HOME", host->DA-DAPPSio_home } }, cmd);
   }
}
...
int main (int argc, char *argv[]) {
  ...
  cli.add_options()
    ...
    ("down", bpo::value<string>(&down_nodes),"comma-separated list of node numbers that will be taken down using the DA-DAPPSio-tn_down.sh script")
    ("bounce", bpo::value<string>(&bounce_nodes),"comma-separated list of node numbers that will be restarted using the DA-DAPPSio-tn_bounce.sh script")
    ("roll", bpo::value<string>(&roll_nodes),"comma-separated list of host names where the nodes should be rolled to a new version using the DA-DAPPSio-tn_roll.sh script")
  ...
  try {
    ...
    if (vmap.count("version") > 0) {
      cout << DA-DAPPSio::launcher::config::version_str << endl;
      return 0;
    }
...
FC_REFLECT( host_def,
            (genesis)(ssh_identity)(ssh_args)(DA-DAPPSio_home)
...
FC_REFLECT( server_name_def, (ipaddr) (name) (has_bios) (DA-DAPPSio_home) (instances) )

FC_REFLECT( server_identities, (producer) (nonprod) (db) (default_DA-DAPPSio_home) (ssh) )
