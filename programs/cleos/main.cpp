 ...
 *  @defgroup eosclienttool DA-DAPPSio Command Line Client Reference
 *  @brief Tool for sending transactions and querying state from @ref nodeos
 *  @ingroup DA-DAPPSclienttool
 */

/**
  @defgroup DA-DAPPSclienttool
  ...
  have a local copy of `nodeos` running and configured to load the 'DA-DAPPSio::chain_api_plugin'.
...
```
$ ./cleos
Command Line Interface to DA-DAPPSio Client
...
#include <DA-DAPPSio/chain/wast_to_wasm.hpp>
...
using namespace DA-DAPPSio::client::help;
using namespace DA-DAPPSio::client::http;
using namespace DA-DAPPSio::client::localize;
using namespace DA-DAPPSio::client::config;
...
string default_wallet_url = "unix://" + (determine_home_directory() / "DA-DAPPSio-wallet" / (string(key_store_executable_name) + ".sock")).string();
...
DA-DAPPSio::client::http::http_context context;
...
fc::variant call( const std::string& url,
                  ...
                  const T& v ) {
   try {
      auto sp = std::make_unique<DA-DAPPSio::client::http::connection_param>(context, parse_url(url) + path, no_verify ? false : true, headers);
      return DA-DAPPSio::client::http::do_http_call(*sp, fc::variant(v), print_request, print_response );
   }
...
DA-DAPPSio::chain_apis::read_only::get_info_results get_info() {
   return call(url, get_info_func).as<DA-DAPPSio::chain_apis::read_only::get_info_results>();
}
...
void print_action( const fc::variant& at ) {
   ...
   /*
   if( code == "DA-DAPPSio" && func == "setcode" )
      args = args.substr(40)+"...";
...
auto abi_serializer_resolver = [](const name& account) -> optional<abi_serializer> {
   ...
   if ( it == abi_cache.end() ) {
      ...
      auto abi_results = result.as<DA-DAPPSio::chain_apis::read_only::get_abi_results>();
...
authority parse_json_authority_or_key(const std::string& authorityJsonOrFile) {
   if (boost::istarts_with(authorityJsonOrFile, "EOS") || boost::istarts_with(authorityJsonOrFile, "PUB_R1")) {
      ...
      DA-DAPPS_ASSERT( DA-DAPPSio::chain::validate(result), authority_type_exception, "Authority failed validation! ensure that keys, accounts, and waits are sorted and that the threshold is valid and satisfiable!");
      return result;
   }
}

asset to_asset( account_name code, const string& s ) {
   static map< pair<account_name, DA-DAPPSio::chain::symbol_code>, DA-DAPPSio::chain::symbol> cache;
   auto a = asset::from_string( s );
   DA-DAPPSio::chain::symbol_code sym = a.get_symbol().to_symbol_code();
   ...
   if ( it == cache.end() ) {
      ...
      if (obj_it != obj.end()) {
         auto result = obj_it->value().as<DA-DAPPSio::chain_apis::read_only::get_currency_stats_result>();
...
inline asset to_asset( const string& s ) {
   return to_asset( N(DA-DAPPSio.token), s );
}
...
struct set_account_permission_subcommand {
   ...
   set_account_permission_subcommand(CLI::App* accountCmd) {
      ...
      permissions->add_flag("--add-code", add_code, localized("[code] add '${code}' permission to specified permission authority", ("code", name(config::DA-DAPPSio_code_name))));
      permissions->add_flag("--remove-code", remove_code, localized("[code] remove '${code}' permission from specified permission authority", ("code", name(config::DA-DAPPSio_code_name))));
      ...
      permissions->set_callback([this] {
         ...
         if ( need_parent || need_auth ) {
            ...
            auto res = json.as<DA-DAPPSio::chain_apis::read_only::get_account_results>();
            ...
            if ( need_auth ) {
               ...
               auto code_name = name(config::DA-DAPPSio_code_name);
...
struct vote_producers_subcommand {
   string voter_str;
   vector<DA-DAPPSio::name> producer_names;
...
struct approve_producer_subcommand {
   DA-DAPPSio::name voter;
   DA-DAPPSio::name producer_name;
   ...
   approve_producer_subcommand(CLI::App* actionRoot) {
      ...
      approve_producer->set_callback([this] {
            ...
            auto res = result.as<DA-DAPPSio::chain_apis::read_only::get_table_rows_result>();
            ...
            DA-DAPPS_ASSERT( 1 == res.rows.size(), multiple_voter_info, "More than one voter_info for account" );
            auto prod_vars = res.rows[0]["producers"].get_array();
            vector<DA-DAPPSio::name> prods;
...
struct list_producers_subcommand {
   ...
   list_producers_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->set_callback([this] {
         ...
         auto result = rawResult.as<DA-DAPPSio::chain_apis::read_only::get_producers_result>();
   ...
   bidname_info_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->set_callback([this] {
         auto rawResult = call(get_table_func, fc::mutable_variant_object("json", true)
                               ("code", "DA-DAPPSio")("scope", "DA-DAPPSio")("table", "namebids")
         ...
         auto result = rawResult.as<DA-DAPPSio::chain_apis::read_only::get_table_rows_result>();
         // Condition in if statement below can simply be res.rows.empty() when cleos no longer needs to support nodeos versions older than 1.5.0
...
void get_account( const string& accountName, const string& coresym, bool json_format ) {
   ...
   if (!json_format) {
      ...
      unordered_map<name, DA-DAPPSio::chain_apis::permission> cache;
      ...
      if( res.refund_request.is_object() ) {
         ...
         if( unstaking > asset( 0, unstaking.get_symbol() ) ) {
            ...
            if( now >= refund_time ) {
               std::cout << " (available to claim now with 'DA-DAPPSio::refund' action)\n";
            } else {
...
int main( int argc, char** argv ) {
   ...
   context = DA-DAPPSio::client::http::create_http_context();
   wallet_url = default_wallet_url;

   CLI::App app{"Command Line Interface to DA-DAPPSio Client"};
   ...
   version->add_subcommand("client", localized("Retrieve version information of the client"))->set_callback([] {
     std::cout << localized("Build version: ${ver}", ("ver", DA-DAPPSio::client::config::version_str)) << std::endl;
   });
   ...
   unpack_action_data->set_callback([&] {
      DA-DAPPS_ASSERT( packed_action_data_string.size() >= 2, transaction_type_exception, "No packed_action_data found" );
...
//                                ->check(CLI::ExistingFile);
   ...
   auto set_code_callback = [&]() {
      ...
      if(!contract_clear){
        ...
        DA-DAPPS_ASSERT( !wasm.empty(), wasm_file_not_found, "no wasm file found ${f}", ("f", wasmPath) );
   ...
   auto set_abi_callback = [&]() {
      ...
      if(!contract_clear){
        ...
        DA-DAPPS_ASSERT( fc::exists( abiPath ), abi_file_not_found, "no abi file found ${f}", ("f", abiPath)  );
   ...
   contractSubcommand->set_callback([&] {
      if(!contract_clear) DA-DAPPS_ASSERT( !contractPath.empty(), contract_exception, " contract-dir is null ", ("f", contractPath) );
      shouldSend = false;
      set_code_callback();
      set_abi_callback();
      if (actions.size()) {
   ...
   propose_action->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, "DA-DAPPSio.msig", "propose", variant_to_bin( N(DA-DAPPSio.msig), N(propose), args ) }});
   });
   ...
   review->set_callback([&] {
      ...
      std::map<DA-DAPPSio::account_name, std::pair<fc::time_point, vector<decltype(all_approvals)::iterator>>>  provided_approvers;
      ...
      if( show_approvals_in_multisig_review ) {
         ...
         if( new_multisig ) {
            for( auto& a : provided_approvers ) {
               ...
               if( rows4.empty() || rows4[0].get_object()["account"].as<DA-DAPPSio::name>() != a.first ) {
                  continue;
               }
   ...
   auto approve_or_unapprove = [&](const string& action) {
      ...
      send_actions({chain::action{accountPermissions, "DA-DAPPSio.msig", action, variant_to_bin( N(DA-DAPPSio.msig), action, args ) }});
   };
   ...
   invalidate->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, "DA-DAPPSio.msig", "invalidate", variant_to_bin( N(DA-DAPPSio.msig), "invalidate", args ) }});
   });
   ...
   cancel->set_callback([&]() {
      ...
      send_actions({chain::action{accountPermissions, "DA-DAPPSio.msig", "cancel", variant_to_bin( N(DA-DAPPSio.msig), N(cancel), args ) }});
      }
   );
   ...
   exec->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, "DA-DAPPSio.msig", "exec", variant_to_bin( N(DA-DAPPSio.msig), N(exec), args ) }});
      }
   );
   ...
   // wrap exec
   string wrap_con = "DA-DAPPSio.wrap";
   executer = "";
   string trx_to_exec;
   ...
   // system subcommand
   auto system = app.add_subcommand("system", localized("Send DA-DAPPSio.system contract action to the blockchain."), false);
