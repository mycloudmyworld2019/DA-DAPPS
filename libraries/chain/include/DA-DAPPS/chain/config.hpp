...
namespace DA-DAPPSio { namespace chain { namespace config {
...
const static uint64_t system_account_name    = N(DA-DAPPSio);
const static uint64_t null_account_name      = N(DA-DAPPSio.null);
const static uint64_t producers_account_name = N(DA-DAPPSio.prods);
...
const static uint64_t DA-DAPPSio_auth_scope       = N(DA-DAPPSio.auth);
const static uint64_t DA-DAPPSio_all_scope        = N(DA-DAPPSio.all);

const static uint64_t active_name = N(active);
const static uint64_t owner_name  = N(owner);
const static uint64_t DA-DAPPSio_any_name = N(DA-DAPPSio.any);
const static uint64_t DA-DAPPSio_code_name = N(DA-DAPPSio.code);
...
const static DA-DAPPSio::chain::wasm_interface::vm_type default_wasm_runtime = DA-DAPPSio::chain::wasm_interface::vm_type::wabt;
...
} } } // namespace DA-DAPPSio::chain::config

constexpr uint64_t DA-DAPPS_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / DA-DAPPSio::chain::config::percent_100;
}

template<typename Number>
Number DA-DAPPS_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + DA-DAPPSio::chain::config::percent_100 - DA-DAPPSio::chain::config::percent_1)  / DA-DAPPSio::chain::config::percent_100;
}
