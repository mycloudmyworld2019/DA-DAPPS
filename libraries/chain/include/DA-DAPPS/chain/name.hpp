...
#define N(X) DA-DAPPSio::chain::string_to_name(#X)
...
namespace std {
   template<> struct hash<DA-DAPPSio::chain::name> : private hash<uint64_t> {
      typedef DA-DAPPSio::chain::name argument_type;
...
namespace fc {
  class variant;
  void to_variant(const DA-DAPPSio::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, DA-DAPPSio::chain::name& check);
} // fc


FC_REFLECT( DA-DAPPSio::chain::name, (value) )
