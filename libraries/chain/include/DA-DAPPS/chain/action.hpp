...
namespace DA-DAPPSio { namespace chain {
    ...
    *  In the context of DA-DAPPSio, every action is dispatched to the handler defined
   ...
   struct action {
      ...
      template<typename T>
      T data_as()const {
         DA-DAPPS_ASSERT( account == T::get_account(), action_type_exception, "account is not consistent with action struct" );
         DA-DAPPS_ASSERT( name == T::get_name(), action_type_exception, "action name is not consistent with action struct"  );
...
FC_REFLECT( DA-DAPPSio::chain::permission_level, (actor)(permission) )
FC_REFLECT( DA-DAPPSio::chain::action, (account)(name)(authorization)(data) )
