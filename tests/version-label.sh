...
# orient ourselves
[[ "$DA-DAPPSio_ROOT" == '' ]] && DA-DAPPSio_ROOT=$(echo $(pwd)/ | grep -ioe '.*/DA-DAPPS/')
[[ "$DA-DAPPSio_ROOT" == '' ]] && DA-DAPPSio_ROOT=$(echo $(pwd)/ | grep -ioe '.*/DA-DAPPSio/DA-DAPPSio/')
[[ "$DA-DAPPSio_ROOT" == '' ]] && DA-DAPPSio_ROOT=$(echo $(pwd)/ | grep -ioe '.*/build/' | sed 's,/build/,,')
echo "Using DA-DAPPSio_ROOT=\"$DA-DAPPSio_ROOT\"."
# determine expected value
CMAKE_CACHE="$DA-DAPPSio_ROOT/build/CMakeCache.txt"
CMAKE_LISTS="$DA-DAPPSio_ROOT/CMakeLists.txt"
if [[ -f "$CMAKE_CACHE" && $(cat "$CMAKE_CACHE" | grep -c 'DOXY_DA-DAPPS_VERSION') > 0 ]]; then
    echo "Parsing \"$CMAKE_CACHE\"..."
    EXPECTED="v$(cat "$CMAKE_CACHE" | grep 'DOXY_DA-DAPPS_VERSION' | cut -d '=' -f 2)"
...
if [[ "$EXPECTED" == '' ]]; then
    ...
    set +e
    echo "DA-DAPPSio_ROOT=\"$DA-DAPPSio_ROOT\""
    ...
    echo ''
    echo '$ cat "$CMAKE_CACHE" | grep "DOXY_DA-DAPPS_VERSION"'
    cat "$CMAKE_CACHE" | grep "DOXY_DA-DAPPS_VERSION"
    echo '$ pwd'
    pwd
    echo '$ ls -la "$DA-DAPPSio_ROOT"'
    ls -la "$DA-DAPPSio_ROOT"
    echo '$ ls -la "$DA-DAPPSio_ROOT/build"'
    ls -la "$DA-DAPPSio_ROOT/build"
    exit 1
fi
echo "Expecting \"$EXPECTED\"..."
# get nodeos version
ACTUAL=$($DA-DAPPSio_ROOT/build/bin/nodeos --version) || : # nodeos currently returns -1 for --version
# test
