/**
 *  @file
 *  @copyright defined in DA-DAPPS/LICENSE.txt
 */
#include <boost/test/unit_test.hpp>
#include <boost/algorithm/string/predicate.hpp>

#include <DA-DAPPSio/testing/tester.hpp>
...
using namespace DA-DAPPSio::testing;
using namespace fc;
...
BOOST_FIXTURE_TEST_CASE( get_scope_test, TESTER ) try {
   ...
   create_accounts({ N(DA-DAPPSio.token), N(DA-DAPPSio.ram), N(DA-DAPPSio.ramfee), N(DA-DAPPSio.stake),
      N(DA-DAPPSio.bpay), N(DA-DAPPSio.vpay), N(DA-DAPPSio.saving), N(DA-DAPPSio.names) });

   std::vector<account_name> accs{N(inita), N(initb), N(initc), N(initd)};
   create_accounts(accs);
   produce_block();

   set_code( N(DA-DAPPSio.token), contracts::DA-DAPPSio_token_wasm() );
   set_abi( N(DA-DAPPSio.token), contracts::DA-DAPPSio_token_abi().data() );
   produce_blocks(1);

   // create currency
   auto act = mutable_variant_object()
         ("issuer",       "DA-DAPPSio")
         ("maximum_supply", DA-DAPPSio::chain::asset::from_string("1000000000.0000 SYS"));
   push_action(N(DA-DAPPSio.token), N(create), N(DA-DAPPSio.token), act );

   // issue
   for (account_name a: accs) {
      push_action( N(DA-DAPPSio.token), N(issue), "DA-DAPPSio", mutable_variant_object()
                  ("to",      name(a) )
                  ("quantity", DA-DAPPSio::chain::asset::from_string("999.0000 SYS") )
                  ("memo", "")
                  );
   }
   produce_blocks(1);
   ...
   // iterate over scope
   DA-DAPPSio::chain_apis::read_only plugin(*(this->control), fc::microseconds::maximum());
   DA-DAPPSio::chain_apis::read_only::get_table_by_scope_params param{N(DA-DAPPSio.token), N(accounts), "inita", "", 10};
   DA-DAPPSio::chain_apis::read_only::get_table_by_scope_result result = plugin.read_only::get_table_by_scope(param);

   BOOST_REQUIRE_EQUAL(4u, result.rows.size());
   BOOST_REQUIRE_EQUAL("", result.more);
   if (result.rows.size() >= 4) {
      BOOST_REQUIRE_EQUAL(name(N(DA-DAPPSio.token)), result.rows[0].code);
      BOOST_REQUIRE_EQUAL(name(N(inita)), result.rows[0].scope);
      BOOST_REQUIRE_EQUAL(name(N(accounts)), result.rows[0].table);
      BOOST_REQUIRE_EQUAL(name(N(DA-DAPPSio)), result.rows[0].payer);
...
BOOST_FIXTURE_TEST_CASE( get_table_test, TESTER ) try {
   ...
   for (account_name a: accs) {
      push_action( N(DA-DAPPSio.token), N(issue), "DA-DAPPSio", mutable_variant_object()
                  ("to",      name(a) )
                  ("quantity", DA-DAPPSio::chain::asset::from_string("10000.0000 SYS") )
                  ("memo", "")
                  );
   }
   produce_blocks(1);
   ...
   act = mutable_variant_object()
         ("issuer",       "DA-DAPPSio")
         ("maximum_supply", DA-DAPPSio::chain::asset::from_string("1000000000.0000 AAA"));
   push_action(N(DA-DAPPSio.token), N(create), N(DA-DAPPSio.token), act );
   // issue
   for (account_name a: accs) {
      push_action( N(DA-DAPPSio.token), N(issue), "DA-DAPPSio", mutable_variant_object()
                  ("to",      name(a) )
                  ("quantity", DA-DAPPSio::chain::asset::from_string("9999.0000 AAA") )
                  ("memo", "")
                  );
   }
   produce_blocks(1);
   ...
   act = mutable_variant_object()
         ("issuer",       "DA-DAPPSio")
         ("maximum_supply", DA-DAPPSio::chain::asset::from_string("1000000000.0000 CCC"));
   push_action(N(DA-DAPPSio.token), N(create), N(DA-DAPPSio.token), act );
   // issue
   for (account_name a: accs) {
      push_action( N(DA-DAPPSio.token), N(issue), "DA-DAPPSio", mutable_variant_object()
                  ("to",      name(a) )
                  ("quantity", DA-DAPPSio::chain::asset::from_string("7777.0000 CCC") )
                  ("memo", "")
                  );
   }
   produce_blocks(1);
   ...
   act = mutable_variant_object()
         ("issuer",       "DA-DAPPSio")
         ("maximum_supply", DA-DAPPSio::chain::asset::from_string("1000000000.0000 BBB"));
   push_action(N(DA-DAPPSio.token), N(create), N(DA-DAPPSio.token), act );
   // issue
   for (account_name a: accs) {
      push_action( N(DA-DAPPSio.token), N(issue), "DA-DAPPSio", mutable_variant_object()
                  ("to",      name(a) )
                  ("quantity", DA-DAPPSio::chain::asset::from_string("8888.0000 BBB") )
                  ("memo", "")
                  );
   }
   produce_blocks(1);
   ...
   DA-DAPPSio::chain_apis::read_only plugin(*(this->control), fc::microseconds::maximum());
   DA-DAPPSio::chain_apis::read_only::get_table_rows_params p;
   p.code = N(DA-DAPPSio.token);
   p.scope = "inita";
   p.table = N(accounts);
   p.json = true;
   p.index_position = "primary";
   DA-DAPPSio::chain_apis::read_only::get_table_rows_result result = plugin.read_only::get_table_rows(p);
   BOOST_REQUIRE_EQUAL(4u, result.rows.size());
   ...
   if (result.rows.size() >= 4) {
      ...
      BOOST_REQUIRE_EQUAL("DA-DAPPSio", result.rows[0]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("DA-DAPPSio", result.rows[1]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("DA-DAPPSio", result.rows[2]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("DA-DAPPSio", result.rows[3]["payer"].as_string());
   }
   p.show_payer = false;
...
BOOST_FIXTURE_TEST_CASE( get_table_by_seckey_test, TESTER ) try {
   ...
   set_code( config::system_account_name, contracts::DA-DAPPSio_system_wasm() );
   set_abi( config::system_account_name, contracts::DA-DAPPSio_system_abi().data() );
   ...
   auto bidname = [this]( const account_name& bidder, const account_name& newname, const asset& bid ) {
      return push_action( N(DA-DAPPSio), N(bidname), bidder, fc::mutable_variant_object()
                          ("bidder",  bidder)
                          ("newname", newname)
                          ("bid", bid)
                          );
   ...
   bidname(N(inita), N(com), DA-DAPPSio::chain::asset::from_string("10.0000 SYS"));
   bidname(N(initb), N(org), DA-DAPPSio::chain::asset::from_string("11.0000 SYS"));
   bidname(N(initc), N(io), DA-DAPPSio::chain::asset::from_string("12.0000 SYS"));
   bidname(N(initd), N(html), DA-DAPPSio::chain::asset::from_string("14.0000 SYS"));
   produce_blocks(1);
