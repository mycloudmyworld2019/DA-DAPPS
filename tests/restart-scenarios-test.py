...
killDA-DAPPSInstances= not args.leave_running
...
try:
    ...
    Print("Stand up DA-DAPPS wallet keosd")
    accountsCount=total_nodes
    walletName="MyWallet"
    Print("Creating wallet %s if one doesn't already exist." % walletName)
    wallet=walletMgr.create(walletName, [cluster.DA-DAPPSioAccount,cluster.defproduceraAccount,cluster.defproducerbAccount])
    ...
    DA-DAPPSioAccount=cluster.DA-DAPPSioAccount
    ...
    if not cluster.createAccounts(DA-DAPPSioAccount):
    ...
    if cluster.killSomeDA-DAPPSInstances(killCount, killSignal) is False:
        errorExit("Failed to kill DA-DAPPS instances")
    ...
    if cluster.relaunchDA-DAPPSInstances(cachePopen=True) is False:
        errorExit("Failed to relaunch DA-DAPPS instances")
    ...
    TestHelper.shutdown(cluster, walletMgr, testSuccessful=testSuccessful, killDA-DAPPSInstances=killEosInstances, killWallet=killDA-DAPPSInstances, keepLogs=keepLogs, cleanRun=killAll, dumpErrorDetails=dumpErrorDetails)
