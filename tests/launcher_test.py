...
try:
    ...
    if not dontLaunch:
        ...
        if cluster.launch(pnodes=pnodes, totalNodes=pnodes, p2pPlugin=p2pPlugin) is False:
            cmdError("launcher")
            errorExit("Failed to stand up DADAPPS cluster.")
    else:
        ...
        if walletMgr.launch() is False:
            ...
            errorExit("Failed to stand up DADAPPS walletd.")
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.DA-DAPPSioAccount,cluster.defproduceraAccount])
    ...
    Print("Create new account %s via %s" % (testeraAccount.name, cluster.DA-DAPPSioAccount.name))
    transId=node.createInitializeAccount(testeraAccount, cluster.DA-DAPPSioAccount, stakedDeposit=0, waitForTransBlock=False, exitOnError=True)

    Print("Create new account %s via %s" % (currencyAccount.name, cluster.DA-DAPPSioAccount.name))
    transId=node.createInitializeAccount(currencyAccount, cluster.DA-DAPPSioAccount, buyRAM=1000000, stakedDeposit=5000, exitOnError=True)

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.DA-DAPPSioAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.DA-DAPPSioAccount, buyRAM=1000000, waitForTransBlock=True, exitOnError=True)
    ...
    actualAmount=node.getAccountDADAPPSBalanceStr(testeraAccount.name)
    ...
    Print("Verify transfer, Expected: %s" % (expectedAmount))
    actualAmount=node.getAccountDADAPPSBalanceStr(currencyAccount.name)
    if expectedAmount != actualAmount:
        cmdError("FAILURE - transfer failed")
        errorExit("Transfer verification failed. Excepted %s, actual: %s" % (expectedAmount, actualAmount))
    ...
    TestHelper.shutdown(cluster, walletMgr, testSuccessful, killDADAPPSInstances, killWallet, keepLogs, killAll, dumpErrorDetails)
