...
try:
    ...
    for account in accounts:
        ...
        trans=node.createInitializeAccount(account, cluster.DA-DAPPSioAccount, stakedDeposit=0, waitForTransBlock=False, stakeNet=1000, stakeCPU=1000, buyRAM=1000, exitOnError=True)
        transferAmount="100000000.0000 {0}".format(CORE_SYMBOL)
        Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.DA-DAPPSioAccount.name, account.name))
        node.transferFunds(cluster.DA-DAPPSioAccount, account, transferAmount, "test transfer")
    ...
    TestHelper.shutdown(cluster, walletMgr, testSuccessful=testSuccessful, killDA-DAPPSInstances=killEosInstances, killWallet=killWallet, keepLogs=keepLogs, cleanRun=killAll, dumpErrorDetails=dumpErrorDetails)
