...
try:
    ...
    if localTest and not dontLaunch:
        ...
        if walletMgr.launch() is False:
            ...
            errorExit("Failed to stand up DA-DAPPS walletd.")
    ...
    if not dontLaunch:
        walletAccounts.append(cluster.DA-DAPPSioAccount)
    ...
    actualAmount=node.getAccountDA-DAPPSBalanceStr(testeraAccount.name)
    ...
    Print("Verify transfer, Expected: %s" % (expectedAmount))
    actualAmount=node.getAccountDA-DAPPSBalanceStr(currencyAccount.name)
    if expectedAmount != actualAmount:
        cmdError("FAILURE - transfer failed")
        errorExit("Transfer verification failed. Excepted %s, actual: %s" % (expectedAmount, actualAmount))
    ...
    contractDir="unittests/contracts/DA-DAPPSio.token"
    wasmFile="DA-DAPPSio.token.wasm"
    abiFile="DA-DAPPSio.token.abi"
    ...
    if not enableMongo:
        ...
        account=node.getDA-DAPPSAccountFromDb(currencyAccount.name)
        ...
        if abiName != "account" or abiActionName != "close" or abiType != "close":
            errorExit("FAILURE - get DA-DAPPS account failed", raw=True)
    ...
    account=node.getDA-DAPPSAccount(defproduceraAccount.name, exitOnError=True)
    ...
    if not enableMongo:
        ...
        rawAccount=node.getDA-DAPPSAccount(defproduceraAccount.name, exitOnError=True, returnType=ReturnType.raw)
    ...
    TestHelper.shutdown(cluster, walletMgr, testSuccessful, killDA-DAPPSInstances, killWallet, keepLogs, killAll, dumpErrorDetails)
