...
class Cluster(object):
    ...
    __BiosPort=8788
    __LauncherCmdArr=[]
    __bootlog="DA-DAPPSio-ignition-wd/bootlog.txt"
    __configDir="etc/DA-DAPPSio/"
    __dataDir="var/lib/"
    ...
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899, enableMongo=False
                 , mongoHost="localhost", mongoPort=27017, mongoDb="DADAPPStest", defproduceraPrvtKey=None, defproducerbPrvtKey=None, staging=False):
        """Cluster container.
        walletd [True|False] Is wallet keosd running. If not load the wallet plugin
        localCluster [True|False] Is cluster local to host.
        host: DADAPPS server host
        port: DADAPPS server port
        walletHost: DADAPPS wallet host
        walletPort: wos wallet port
        enableMongo: Include mongoDb support, configures DADAPPS mongo plugin
        ...
        self.DA-DAPPSioAccount=self.defProducerAccounts["DA-DAPPSio"]= Account("DA-DAPPSio")
    ...
    def launch(self, pnodes=1, totalNodes=1, prodCount=1, topo="mesh", p2pPlugin="net", delay=1, onlyBios=False, dontBootstrap=False,
               totalProducers=None, extraNodDADAPPSArgs=None, useBiosBootFile=True, specificExtraNodDADAPPSArgs=None):
        """Launch cluster.
        ...
        extraNodDADAPPSArgs: string of arguments to pass through to each nodoes instance (via --nodeos flag on launcher)
        ...
        specificExtraNodDADAPPSArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-nodeos flags on launcher), example: { "5" : "--plugin DA-DAPPSio::test_control_api_plugin" }
        """
        assert(isinstance(topo, str))

        if not self.localCluster:
            Utils.Print("WARNING: Cluster not local, not launching %s." % (Utils.DADAPPSServerName))
            return True
        ...
        cmd="%s -p %s -n %s -d %s -i %s -f --p2p-plugin %s %s" % (
            Utils.DADAPPSLauncherPath, pnodes, totalNodes, delay, datetime.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3],
        ...
        if not self.walletd:
            nodDADAPPSArgs += " --plugin DA-DAPPSio::wallet_api_plugin"
        if self.enableMongo:
            nodDADAPPSArgs += " --plugin DA-DAPPSio::mongo_db_plugin --mongodb-wipe --delete-all-blocks --mongodb-uri %s" % self.mongoUri
        if extraNodDADAPPSArgs is not None:
            assert(isinstance(extraNodDADAPPSArgs, str))
            nodDADAPPSArgs += extraNodDADAPPSArgs
        if Utils.Debug:
            nodDADAPPSArgs += " --contracts-console"

        if nodDADAPPSArgs:
            cmdArr.append("--nodeos")
            cmdArr.append(nodDADAPPSArgs)

        if specificExtraNodDADAPPSArgs is not None:
            assert(isinstance(specificExtraNodDADAPPSArgs, dict))
            for nodeNum,arg in specificExtraNodDADAPPSArgs.items():
        ...
        if nodes is None or totalNodes != len(nodes):
            Utils.Print("ERROR: Unable to validate %s instances, expected: %d, actual: %d" %
                          (Utils.DADAPPSServerName, totalNodes, len(nodes)))
            return False
        ...
        self.DA-DAPPSioAccount=self.defProducerAccounts["DA-DAPPSio"]
    ...
    def getClientVersion(verbose=False):
        ...
        try:
            cmd="%s version client" % (Utils.DADAPPSClientPath)
    ...
    def createAccountKeys(count):
        ...
        for _ in range(0, count):
            try:
                cmd="%s create key --to-console" % (Utils.DADAPPSClientPath)
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        fromm=source
        to=accounts[0]
        Utils.Print("Transfer %s units from account %s to %s on DADAPPS server port %d" % (
        ...
        for i in range(0, count):
            ...
            for _ in range(0, count):
                #Utils.Print("nextDADAPPSIdx: %d, n: %d" % (nextDADAPPSIdx, n))
                nextDADAPPSIdx=(nextDADAPPSIdx + 1)%count
                if not self.nodes[nextDADAPPSIdx].killed:
                    #Utils.Print("nextDADAPPSIdx: %d" % (nextDADAPPSIdx))
            ...
            #Utils.Print("nextDADAPPSIdx: %d, count: %d" % (nextDADAPPSIdx, count))
            node=self.nodes[nextDADAPPSIdx]
    ...
    def validateSpreadFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        for node in self.nodes:
            ...
            if Utils.Debug: Utils.Print("Validate funds on %s server port %d." %
                                        (Utils.DADAPPSServerName, node.port))

            if node.validateFunds(initialBalances, transferAmount, source, accounts) is False:
                Utils.Print("ERROR: Failed to validate funds on DADAPPS node port: %d" % (node.port))
                return False
    ...
    def spreadFundsAndValidate(self, transferAmount=1):
        ...
        initialBalances=self.nodes[0].getDADAPPSBalances([self.defproduceraAccount] + self.accounts)
        assert(initialBalances)
        assert(isinstance(initialBalances, dict))
    ...
    def validateAccounts(self, accounts, testSysAccounts=True):
        ...
        myAccounts = []
        if testSysAccounts:
            myAccounts += [self.DA-DAPPSioAccount, self.defproduceraAccount, self.defproducerbAccount]
        if accounts:
            assert(isinstance(accounts, list))
            myAccounts += accounts
    ...
    def parseClusterKeys(totalNodes):
        ...
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse DA-DAPPSio private keys from cluster config files.")
            return None
    ...
    def bios_bootstrap(totalNodes, biosHost, biosPort, walletMgr, silent=False):
        """Bootstrap cluster using the bios_boot.sh script generated by DA-DAPPSio-launcher."""
        ...
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            if not silent: Utils.Print("Launcher failed to shut down DADAPPS cluster.")
            return None
        ...
        DA-DAPPSioName="DA-DAPPSio"
        DA-DAPPSioKeys=producerKeys[DA-DAPPSioName]
        DA-DAPPSioAccount=Account(DA-DAPPSioName)
        DA-DAPPSioAccount.ownerPrivateKey=DA-DAPPSioKeys["private"]
        DA-DAPPSioAccount.ownerPublicKey=DA-DAPPSioKeys["public"]
        DA-DAPPSioAccount.activePrivateKey=DA-DAPPSioKeys["private"]
        DA-DAPPSioAccount.activePublicKey=DA-DAPPSioKeys["public"]
        producerKeys.pop(DA-DAPPSioName)

        if not walletMgr.importKey(DA-DAPPSioAccount, ignWallet):
            Utils.Print("ERROR: Failed to import %s account keys into ignition wallet." % (DA-DAPPSioName))
            return None

        initialFunds="1000000.0000 {0}".format(CORE_SYMBOL)
        Utils.Print("Transfer initial fund %s to individual accounts." % (initialFunds))
        trans=None
        contract="DA-DAPPSio.token"
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"DA-DAPPSio\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (name, initialFunds, "init DA-DAPPSio transfer")
            opts="--permission DA-DAPPSio@active"
            if name != "DA-DAPPSio":
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from DA-DAPPSio.token to %s." % (name))
                    return None
    ...
    def bootstrap(totalNodes, prodCount, totalProducers, biosHost, biosPort, walletMgr, onlyBios=False):
        ...
        contract="DA-DAPPSio.bios"
        contractDir="unittests/contracts/%s" % (contract)
        wasmFile="%s.wasm" % (contract)
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        trans=biosNode.publishContract(DA-DAPPSioAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
        ...
        for name, keys in producerKeys.items():
            ...
            trans=biosNode.createAccount(initx, DA-DAPPSioAccount, 0)
            if trans is None:
        ...
        if not onlyBios:
            if prodCount == -1:
                ...
                with open(setProdsFile, "r") as f:
                    ...
                    opts="--permission DA-DAPPSio@active"
                    myTrans=biosNode.pushMessage("DA-DAPPSio", "setprods", setProdsStr, opts)
                ...
                trans=biosNode.pushMessage("DA-DAPPSio", "setprods", setProdsStr, opts)
                if trans is None or not trans[0]:
            ...
            # wait for block production handover (essentially a block produced by anyone but DA-DAPPSio).
            lam = lambda: biosNode.getInfo(exitOnError=True)["head_block_producer"] != "DA-DAPPSio"
            ret=Utils.waitForBool(lam)
            if not ret:
                Utils.Print("ERROR: Block production handover failed.")
                return None

        DA-DAPPSioTokenAccount=copy.deepcopy(DA-DAPPSioAccount)
        DA-DAPPSioTokenAccount.name="DA-DAPPSio.token"
        trans=biosNode.createAccount(DA-DAPPSioTokenAccount, DA-DAPPSioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (DA-DAPPSioTokenAccount.name))
            return None

        DA-DAPPSioRamAccount=copy.deepcopy(DA-DAPPSioAccount)
        DA-DAPPSioRamAccount.name="DA-DAPPSio.ram"
        trans=biosNode.createAccount(DA-DAPPSioRamAccount, DA-DAPPSioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (DA-DAPPSioRamAccount.name))
            return None

        DA-DAPPSioRamfeeAccount=copy.deepcopy(DA-DAPPSioAccount)
        DA-DAPPSioRamfeeAccount.name="DA-DAPPSio.ramfee"
        trans=biosNode.createAccount(DA-DAPPSioRamfeeAccount, DA-DAPPSioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (DA-DAPPSioRamfeeAccount.name))
            return None

        DA-DAPPSioStakeAccount=copy.deepcopy(DA-DAPPSioAccount)
        DA-DAPPSioStakeAccount.name="DA-DAPPSio.stake"
        trans=biosNode.createAccount(DA-DAPPSioStakeAccount, DA-DAPPSioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (DA-DAPPSioStakeAccount.name))
            return None
        ...
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        trans=biosNode.publishContract(DA-DAPPSioTokenAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
            Utils.Print("ERROR: Failed to publish contract %s." % (contract))
            return None

        # Create currency0000, followed by issue currency0000
        contract=DA-DAPPSioTokenAccount.name
        Utils.Print("push create action to %s contract" % (contract))
        action="create"
        data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\",\"can_freeze\":\"0\",\"can_recall\":\"0\",\"can_whitelist\":\"0\"}" % (DA-DAPPSioTokenAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (contract)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push create action to DA-DAPPSio contract.")
            return None
        ...
        action="issue"
        data="{\"to\":\"%s\",\"quantity\":\"1000000000.0000 %s\",\"memo\":\"initial issue\"}" % (DA-DAPPSioAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (contract)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push issue action to DA-DAPPSio contract.")
            return None
        ...
        Utils.Print("Verify DA-DAPPSio issue, Expected: %s" % (expectedAmount))
        actualAmount=biosNode.getAccountDADAPPSBalanceStr(DA-DAPPSioAccount.name)
        if expectedAmount != actualAmount:
            Utils.Print("ERROR: Issue verification failed. Excepted %s, actual: %s" %
                        (expectedAmount, actualAmount))
            return None

        contract="DA-DAPPSio.system"
        contractDir="unittests/contracts/%s" % (contract)
        wasmFile="%s.wasm" % (contract)
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        ...
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"%s\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (DA-DAPPSioAccount.name, name, initialFunds, "init transfer")
            opts="--permission %s@active" % (DA-DAPPSioAccount.name)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to transfer funds from %s to %s." % (DA-DAPPSioTokenAccount.name, name))
                return None
        ...
        opts="--permission %s@active" % (DA-DAPPSioAccount.name)
        trans=biosNode.pushMessage(DA-DAPPSioAccount.name, action, data, opts)
        Utils.Print("Cluster bootstrap done.")

        return biosNode

    @staticmethod
    def pgrepDADAPPSServers(timeout=None):
        cmd=Utils.pgrepCmd(Utils.DADAPPSServerName)
    ...
    @staticmethod
    def pgrepDADAPPSServerPattern(nodeInstance):
        dataLocation=Cluster.__dataDir + Cluster.nodeExtensionToName(nodeInstance)
        return r"[\n]?(\d+) (.* --data-dir %s .*)\n" % (dataLocation)

    # Populates list of DADAPPSInstanceInfo objects, matched to actual running instances
    def discoverLocalNodes(self, totalNodes, timeout=None):
        nodes=[]

        psOut=Cluster.pgrepDADAPPSServers(timeout)
        if psOut is None:
        ...
        for i in range(0, totalNodes):
            pattern=Cluster.pgrepDADAPPSServerPattern(i)
            m=re.search(pattern, psOut, re.MULTILINE)
            if m is None:
                Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.DADAPPSServerName, pattern))
                break
    ...
    def discoverBiosNodePid(self, timeout=None):
        psOut=Cluster.pgrepDADAPPSServers(timeout=timeout)
        pattern=Cluster.pgrepDADAPPSServerPattern("bios")
        Utils.Print("pattern={\n%s\n}, psOut=\n%s\n" % (pattern,psOut))
        m=re.search(pattern, psOut, re.MULTILINE)
        if m is None:
            Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.DADAPPSServerName, pattern))
    ...
    # Kills a percentange of DADAPPS instances starting from the tail and update DADAPPSInstanceInfos state
    def killSomeDADAPPSInstances(self, killCount, killSignalStr=Utils.SigKillTag):
        killSignal=signal.SIGKILL
        if killSignalStr == Utils.SigTermTag:
            killSignal=signal.SIGTERM
        Utils.Print("Kill %d %s instances with signal %s." % (killCount, Utils.DADAPPSServerName, killSignal))
    ...
    def relaunchDADAPPSInstances(self, cachePopen=False):
    ...
    def killall(self, silent=True, allInstances=False):
        ...
        cmd="%s -k 9" % (Utils.DADAPPSLauncherPath)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            if not silent: Utils.Print("Launcher failed to shut down DADAPPS cluster.")

        if allInstances:
            # ocassionally the launcher cannot kill the DADAPPS server
            cmd="pkill -9 %s" % (Utils.DADAPPSServerName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
                if not silent: Utils.Print("Failed to shut down DADAPPS cluster.")
