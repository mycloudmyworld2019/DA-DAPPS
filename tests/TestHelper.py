...
class TestHelper(object):
    ...
    def parse_args(includeArgs, applicationSpecificArgs=AppArgs()):
        ...
        if "--host" in includeArgs:
            parser.add_argument("-h", "--host", type=str, help="%s host name" % (Utils.DA-DAPPSServerName),
                                     default=TestHelper.LOCAL_HOST)
        if "--port" in includeArgs:
            parser.add_argument("-p", "--port", type=int, help="%s host port" % Utils.DA-DAPPSServerName,
                                     default=TestHelper.DEFAULT_PORT)
        if "--wallet-host" in includeArgs:
            parser.add_argument("--wallet-host", type=str, help="%s host" % Utils.DA-DAPPSWalletName,
                                     default=TestHelper.LOCAL_HOST)
        if "--wallet-port" in includeArgs:
            parser.add_argument("--wallet-port", type=int, help="%s port" % Utils.DA-DAPPSWalletName,
        ...
        if "--dump-error-details" in includeArgs:
            parser.add_argument("--dump-error-details",
                                     help="Upon error print etc/DA-DAPPSio/node_*/config.ini and var/lib/node_*/stderr.log to stdout",
                                     action='store_true')
    ...
    def printSystemInfo(prefix):
        ...
        Utils.Print("DA-DAPPS Client version: %s" % (clientVersion))
        Utils.Print("Processor: %s" % (platform.processor()))
        Utils.Print("OS name: %s" % (platform.platform()))
    
    @staticmethod
    # pylint: disable=too-many-arguments
    def shutdown(cluster, walletMgr, testSuccessful=True, killDA-DAPPSInstances=True, killWallet=True, keepLogs=False, cleanRun=True, dumpErrorDetails=False):
        ...
        assert(isinstance(killDA-DAPPSInstances, bool))
        ...
        if not testSuccessful and dumpErrorDetails:
            ...
            psOut=Cluster.pgrepDA-DAPPSServers(timeout=60)
