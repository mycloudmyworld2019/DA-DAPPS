...
read -d '' genesis << EOF
{
  ...
  "initial_key": "DADAPPS6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV",
...
plugin = DA-DAPPSio::net_plugin
plugin = DA-DAPPSio::history_api_plugin
http-server-address = 127.0.0.1:8888
blocks-dir = blocks
p2p-listen-endpoint = 0.0.0.0:9876
allowed-connection = any
private-key = ['DADAPPS6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV','5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3']
send-whole-blocks = true
readonly = 0
p2p-max-nodes-per-host = 10
enable-stale-production = true
producer-name = DA-DAPPSio
EOF
...
rm -rf staging
rm -rf etc/DA-DAPPSio/node_*
rm -rf var/lib
cName=config.ini
lName=logging.json
gName=genesis.json

path=staging/etc/DA-DAPPSio/node_bios
mkdir -p $path
echo "$configbios" > $path/$cName
echo "$loggingbios" > $path/$lName
echo "$genesis" > $path/$gName

path=staging/etc/DA-DAPPSio/node_00
mkdir -p $path
echo "$config00" > $path/$cName
echo "$logging00" > $path/$lName
echo "$genesis" > $path/$gName

path=staging/etc/DA-DAPPSio/node_01
mkdir -p $path
...
programs/DA-DAPPSio-launcher/DA-DAPPSio-launcher -p $pnodes -n $total_nodes --nogen -d $delay
...
programs/DA-DAPPSio-launcher/DA-DAPPSio-launcher -k 15
rm -rf staging
rm -rf var/lib/node_*
rm -rf etc/DA-DAPPSio/node_*
exit $ret
