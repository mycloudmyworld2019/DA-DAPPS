...
# DA-DAPPSio should have the same key as defproducera
DA-DAPPSio = copy.copy(defproduceraAccount)
DA-DAPPSio.name = "DA-DAPPSio"

Print("Info of each node:")
for i in range(len(hosts)):
    node = node0
    cmd="%s %s get info" % (testUtils.Utils.DA-DAPPSClientPath, node.endpointArgs)
    trans = node.runCmdReturnJson(cmd)
    Print("host %s: %s" % (hosts[i], trans))


wasmFile="DA-DAPPSio.system.wasm"
abiFile="DA-DAPPSio.system.abi"
Print("\nPush system contract %s %s" % (wasmFile, abiFile))
trans=node0.publishContract(DA-DAPPSio.name, wasmFile, abiFile, waitForTransBlock=True)
if trans is None:
    Utils.errorExit("Failed to publish DA-DAPPSio.system.")
else:
...
try:
    ...
    for cmdInd in range(maxIndex):
        (transIdList, checkacct, expBal, errmsg) = module.execute(cmdInd, node0, testeraAccount, DA-DAPPSio)
