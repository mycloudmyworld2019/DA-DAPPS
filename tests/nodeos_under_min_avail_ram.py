...
killDA-DAPPSInstances=not dontKill
killWallet=not dontKill

WalletdName=Utils.DA-DAPPSWalletName
ClientName="cleos"
...
try:
    ...
    maxRAMValue=1010
    extraNodDA-DAPPSArgs=" %s %d %s %d " % (minRAMFlag, minRAMValue, maxRAMFlag, maxRAMValue)
    if cluster.launch(onlyBios=False, pnodes=totalNodes, totalNodes=totalNodes, totalProducers=totalNodes, extraNodDA-DAPPSArgs=extraNodeosArgs, useBiosBootFile=False) is False:
        Utils.cmdError("launcher")
        errorExit("Failed to stand up DA-DAPPS cluster.")
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.DA-DAPPSioAccount])
    ...
    # create accounts via DA-DAPPSio as otherwise a bid is needed
    for account in accounts:
        Print("Create new account %s via %s" % (account.name, cluster.DA-DAPPSioAccount.name))
        trans=nodes[0].createInitializeAccount(account, cluster.DA-DAPPSioAccount, stakedDeposit=500000, waitForTransBlock=False, stakeNet=50000, stakeCPU=50000, buyRAM=50000, exitOnError=True)
        transferAmount="70000000.0000 {0}".format(CORE_SYMBOL)
        Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.DA-DAPPSioAccount.name, account.name))
        nodes[0].transferFunds(cluster.DA-DAPPSioAccount, account, transferAmount, "test transfer")
    ...
    Print("Create new account %s via %s" % (contractAccount.name, cluster.DA-DAPPSioAccount.name))
    trans=nodes[0].createInitializeAccount(contractAccount, cluster.DA-DAPPSioAccount, stakedDeposit=500000, waitForTransBlock=False, stakeNet=50000, stakeCPU=50000, buyRAM=50000, exitOnError=True)
    transferAmount="90000000.0000 {0}".format(CORE_SYMBOL)
    Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.DA-DAPPSioAccount.name, contractAccount.name))
    nodes[0].transferFunds(cluster.DA-DAPPSioAccount, contractAccount, transferAmount, "test transfer")
        ...
        for fromIndex in range(namedAccounts.numAccounts):
            ...
            try:
                ...
                if trans is None or not trans[0]:
                    timeOutCount+=1
                    if timeOutCount>=3:
                       Print("Failed to push create action to DA-DAPPSio contract for %d consecutive times, looks like nodeos already exited." % (timeOutCount))
                       keepProcessing=False
                       break
                    Print("Failed to push create action to DA-DAPPSio contract. sleep for 60 seconds")
                    time.sleep(60)
                else:
                    timeOutCount=0
                time.sleep(1)
    ...
    TestHelper.shutdown(cluster, walletMgr, testSuccessful=testSuccessful, killDA-DAPPSInstances=killDA-DAPPSInstances, killWallet=killWallet, keepLogs=keepLogs, cleanRun=killAll, dumpErrorDetails=dumpErrorDetails)
