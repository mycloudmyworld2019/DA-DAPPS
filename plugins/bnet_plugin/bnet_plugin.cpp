...
#include <DA-DAPPSio/bnet_plugin/bnet_plugin.hpp>
...
using DA-DAPPSio::public_key_type;
using DA-DAPPSio::chain_id_type;
using DA-DAPPSio::block_id_type;
using DA-DAPPSio::block_timestamp_type;
using std::string;
using DA-DAPPSio::sha256;
using DA-DAPPSio::signed_block_ptr;
using DA-DAPPSio::packed_transaction_ptr;
using std::vector;
...
namespace DA-DAPPSio {
       ...
       //    shared_ptr< vector<char> >  block_msg; ///< packed bnet_message for this block
        ...
        void on( const signed_block_ptr& b ) {
           ...
           if (!b) {
              ...
              DA-DAPPS_THROW(block_validate_exception, "bad block" );
           }
  ...
  class listener : public std::enable_shared_from_this<listener> {
     ...
     public:
        ...
        void run() {
           DA-DAPPS_ASSERT( _acceptor.is_open(), plugin_exception, "unable top open listen socket" );
           do_accept();
        }
   ...
   void bnet_plugin::plugin_shutdown() {
      ...
      my->for_each_session([](auto ses){
         DA-DAPPS_ASSERT( false, plugin_exception, "session ${ses} still active", ("ses", ses->_session_num) );
      });
   ...
   void session::on( const packed_transaction_ptr& p ) {
      ...
      if (!p) {
        ...
        DA-DAPPS_THROW(transaction_exception, "bad transaction");
      }
