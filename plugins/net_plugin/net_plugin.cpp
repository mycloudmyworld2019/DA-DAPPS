...
#include <DA-DAPPSio/net_plugin/net_plugin.hpp>
#include <DA-DAPPSio/net_plugin/protocol.hpp>
...
using namespace DA-DAPPSio::chain::plugin_interface::compat;
...
namespace DA-DAPPSio {
   ...
   using DA-DAPPSio::chain::transaction_id_type;
...
#define peer_elog( PEER, FORMAT, ... ) \
  ...
  FC_MULTILINE_MACRO_END
   ...
   typedef multi_index_container<
      DA-DAPPSio::peer_block_state,
      indexed_by<
         ordered_unique< tag<by_id>, member<DA-DAPPSio::peer_block_state, block_id_type, &DA-DAPPSio::peer_block_state::id >, sha256_less >,
         ordered_unique< tag<by_block_num>, member<DA-DAPPSio::peer_block_state, uint32_t, &DA-DAPPSio::peer_block_state::block_num > >
         >
   ...
   public:
      ...
      void fill_out_buffer( std::vector<boost::asio::const_buffer>& bufs ) {
         if( _sync_write_queue.size() > 0 ) { // always send msgs from sync_write_queue first
            ...
            DA-DAPPS_ASSERT( _write_queue_size == 0, plugin_exception, "write queue size expected to be zero" );
         }
      }
   ...
   struct msg_handler : public fc::visitor<void> {
      ...
      void operator()( const signed_block& msg ) const {
         DA-DAPPS_ASSERT( false, plugin_config_exception, "operator()(signed_block&&) should be called" );
      }
      void operator()( signed_block& msg ) const {
         DA-DAPPS_ASSERT( false, plugin_config_exception, "operator()(signed_block&&) should be called" );
      }
      void operator()( const packed_transaction& msg ) const {
         DA-DAPPS_ASSERT( false, plugin_config_exception, "operator()(packed_transaction&&) should be called" );
      }
            ...
            app().post( priority::medium, [this,weak_conn, ec, bytes_transferred]() {
               ...
               try {
                  if( !ec ) {
                     ...
                     DA-DAPPS_ASSERT(bytes_transferred <= conn->pending_message_buffer.bytes_to_write(), plugin_exception, "");
   ...
   void net_plugin_impl::handle_message(const connection_ptr& c, const packed_transaction_ptr& trx) {
      ...
      controller& cc = my_impl->chain_plug->chain();
      if( cc.get_read_mode() == DA-DAPPSio::db_read_mode::READ_ONLY ) {
   ...
   void net_plugin::set_program_options( options_description& /*cli*/, options_description& cfg )
   {
      cfg.add_options()
         ...
         ( "agent-name", bpo::value<string>()->default_value("\"DA-DAPPS Test Agent\""), "The name supplied to identify this node amongst the peers.")
   ...
   void net_plugin::plugin_initialize( const variables_map& options ) {
      ...
      try {
         ...
         if( my->allowed_connections & net_plugin_impl::Specified )
            DA-DAPPS_ASSERT( options.count( "peer-key" ),
         ...
         DA-DAPPS_ASSERT( my->chain_plug, chain::missing_chain_plugin_exception, ""  );
